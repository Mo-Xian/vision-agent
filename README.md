...
阶段1 数据采集:
  ...
  手机: MobileRecorder(scrcpy + ADB 触控) → recording.mp4 + actions.jsonl

阶段2 行为克隆 (BC):
  LLM Coach 动作发现 → MobileNetV3 编码(576维) → E2EMLP 训练 → 初始策略

阶段3 数据扩展:
  伪标签扩展(用模型标注新视频, 高置信度样本扩充) → 重新训练

阶段4 RL 自对弈:
  BC 模型热启动 → GameEnvironment(scrcpy 截屏 + ADB 操作)
  → DQN Agent(ε-greedy) → RewardDetector(血条/击杀/胜负)
  → 经验回放 → Q 网络训练 → 持续改进

部署:
  DQNEngine / E2EEngine → ADB 触控(手机) 或 pynput(PC) → 实时执行
```
...
| 路径 | 输入 | 方法 | 产出 |
|------|------|------|------|
| **A. 行为克隆** | 人工录制的操作视频 | MobileNetV3 + MLP 监督学习 | `model.pt` (e2e_mlp) |
| **B. 伪标签扩展** | 已有模型 + 新视频 | 模型推理高置信度帧 → 扩充训练集 | 更强的 `model.pt` |
| **C. RL 自对弈** | BC 模型(可选) + 手机设备 | DQN + ε-greedy 探索 + 经验回放 | `model.pt` (dqn) |

三条路径产出的模型格式统一（`model.pt` + `model.meta.json`），可互相作为输入：
- A 的产出可用于 B 扩展和 C 热启动
- C 的产出可用于 B 伪标签扩展
- 模型浏览器自动扫描所有产出，一目了然
