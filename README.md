...
| **标注纠错** | 在可视化回放中直接修改动作标注、删除坏样本，保存修正后的 JSONL |
| **标注 A/B 对比** | 加载两份标注文件，逐帧对比决策差异，统计一致率 |
| **多视频批量标注** | 选择多个视频一次性标注，自动生成独立输出文件 |
| **训练曲线可视化** | 训练过程中实时显示 Loss / Train Acc / Val Acc 曲线 |
| **数据集质量分析** | 训练前分析样本数量、动作分布均衡度，提示潜在问题 |
| **帧率控制** | 可配置目标 FPS，防止 CPU/GPU 过载（config.yaml `pipeline.target_fps`） |
...
| **配置导入/导出** | 一键导出/导入配置和 Profiles 为 ZIP 压缩包，方便分享 |
